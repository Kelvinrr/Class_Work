...
int main(int argc, char *argv[]) {
  ...
  if (strcmp(file_format, "P3") == 0) {
    if (!(size = check_ascii_image_size(width, height, buffer, INIT_BUFFER_SIZE,
                                        input_fd))) {
      fprintf(stderr, "ERROR: Real image size does not match header.\n");
      fclose(output_file);
      fclose(input_fd);
...
void bin_to_ascii(char *buffer, size_t width, size_t height, FILE *input_file,
                  FILE *output_file) {
...
void ascii_to_bin(char *buffer, size_t size, FILE *input_file,
                  FILE *output_file) {
  char *token = NULL;
